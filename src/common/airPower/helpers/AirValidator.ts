 ...
 * @author Hamm
 */
import { AirInputType } from '../enums/AirInputType'
import type { IValidateRule } from '../intetface/IValidateRule'
...
export class AirValidator {
  ...
  /**
   * # 验证是否是简易手机号码 .e.q. 1xxxxxxxxxx
   * @param num 号码
   */
  static isMobileSimplePhone(num: string): boolean {
  ...
  static isChineseIdCard(str: string): boolean {
    ...
    if (str.length) {
      if (str.length === 18) {
        const year = Number.parseInt(str.substring(6), this.DEFAULT_RADIX)
        if (year > new Date().getFullYear() || year < 1900)
          return false

        const month = Number.parseInt(str.substring(10, 12), this.DEFAULT_RADIX)
        if (month > 12 || month < 1)
          return false

        const day = Number.parseInt(str.substring(12, 14), this.DEFAULT_RADIX)
        if (day > 31 || month < 1)
          return false
        ...
        for (let i = 0; i < 17; i += 1)
          sum += Number.parseInt(str[i], this.DEFAULT_RADIX) * (arr[0][i] as number)
