  ...
  <view style="height: 100%;">
    <view :class="showUpdateTime ? 'zp-r-container zp-r-container-padding' : 'zp-r-container'">
      <view class="zp-r-left">
        ...
        <!-- 加载状态图片 -->
        <!-- #ifndef APP-NVUE -->
        <image v-else class="zp-r-left-image" :class="{ 'zp-line-loading-image': refreshingAnimated, 'zp-r-left-image-pre-size-rpx': unit === 'rpx', 'zp-r-left-image-pre-size-px': unit === 'px' }" :style="[leftImageStyle, imgStyle]" :src="leftImageSrc" />
        <!-- #endif -->
        <!-- 在nvue中，加载状态loading使用系统loading -->
        <!-- #ifdef APP-NVUE -->
        <view v-else :style="[{ 'margin-right': showUpdateTime ? addUnit(18, unit) : addUnit(12, unit) }]">
          <loading-indicator
            :class="isIos ? { 'zp-loading-image-ios-rpx': unit === 'rpx', 'zp-loading-image-ios-px': unit === 'px' } : { 'zp-loading-image-android-rpx': unit === 'rpx', 'zp-loading-image-android-px': unit === 'px' }"
